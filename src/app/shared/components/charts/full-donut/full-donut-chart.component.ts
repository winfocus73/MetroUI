...
@Component({
    selector: 'nxasm-full-donut-chart',
    encapsulation: ViewEncapsulation.None,
    templateUrl: './full-donut-chart.component.html',
    styleUrls: ['./full-donut-chart.component.scss']
})
export class FullDonutChartComponent implements OnInit, OnChanges {
    @Input() data: any[]=[];
    ...
    total: number=0; // Total of chart values
    ...
    .endAngle(6)
    .sort(null);
    //.value(this.data.reduce(function(a, b) { return a + b; }, 0));
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        this.total=this.data.reduce(function(a, b) { return a + b; }, 0);
    }
    ...
    private setChartDimensions() {
        ...
        this.svg = d3.select(this.hostElement).append('svg')
            .attr('width', this.size ? this.size.width : '100%')
            .attr('height', this.size ? this.size.height : '100%')
    ...
    private updateLabels() {
        ...
        this.labels.each((datum:any, index:any, n:any) => {
            d3.select(n[index]).text(this.labelValueFn(this.rawData[index],index));
        });
    ...
    private labelValueGetter = (datum:any, index:any) => {
        return this.labelValueFn(this.rawData[index],index);
    }

    private labelValueFn(val:any,index:any) {
        const pct = Math.floor(val * 100 / this.total);
        return (pct < 4) ? '' : (this.data[index].Label+','+this.data[index].Count);
    }
